(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))a(i);new MutationObserver(i=>{for(const c of i)if(c.type==="childList")for(const s of c.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&a(s)}).observe(document,{childList:!0,subtree:!0});function r(i){const c={};return i.integrity&&(c.integrity=i.integrity),i.referrerPolicy&&(c.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?c.credentials="include":i.crossOrigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function a(i){if(i.ep)return;i.ep=!0;const c=r(i);fetch(i.href,c)}})();const tt={isLoggedIn:()=>!!localStorage.getItem("nx_token"),getUser:()=>{try{return JSON.parse(localStorage.getItem("nx_user")||"{}")}catch{return{}}},setSession(t,e){localStorage.setItem("nx_token",t),localStorage.setItem("nx_user",JSON.stringify(e))},clear(){localStorage.removeItem("nx_token"),localStorage.removeItem("nx_user")}},ot={};function T(t,e){ot[t]=e}function et(t){window.location.hash=t}function Lt(){window.addEventListener("hashchange",mt),mt()}function mt(){const t=window.location.hash.replace("#","")||"dashboard";if(!tt.isLoggedIn()&&t!=="login"){et("login");return}if(tt.isLoggedIn()&&t==="login"){et("dashboard");return}const e=ot[t];e?e():ot[404]&&ot[404]()}const Bt="/api/v1";function Pt(){return localStorage.getItem("nx_token")}class jt extends Error{constructor(e,r){super(r),this.status=e}}async function $(t,e,r){const a=Pt(),i=await fetch(Bt+e,{method:t,headers:{"Content-Type":"application/json",...a?{Authorization:`Bearer ${a}`}:{}},...r!==void 0?{body:JSON.stringify(r)}:{}});if(i.status===401)return localStorage.removeItem("nx_token"),localStorage.removeItem("nx_user"),window.location.hash="login",null;if(!i.ok)throw new jt(i.status,await i.text());return(i.headers.get("content-type")||"").includes("application/json")?i.json():i.text()}const h={get:t=>$("GET",t),post:(t,e)=>$("POST",t,e),put:(t,e)=>$("PUT",t,e),del:t=>$("DELETE",t),login:(t,e)=>$("POST","/auth/login",{login:t,password:e}),logout:()=>$("POST","/auth/logout",{}),dashboard:()=>$("GET","/dashboard"),ventaKpis:()=>$("GET","/ventas/kpis"),factKpis:()=>$("GET","/facturas/kpis"),stockKpis:()=>$("GET","/stock/kpis"),ventas:(t=1)=>$("GET",`/ventas?pagina=${t}`),venta:t=>$("GET",`/ventas/${t}`),facturas:(t=1)=>$("GET",`/facturas?pagina=${t}`),factura:t=>$("GET",`/facturas/${t}`),porCobrar:()=>$("GET","/facturas/por-cobrar"),productos:(t=1,e="")=>$("GET",`/productos?pagina=${t}&q=${encodeURIComponent(e)}`),producto:t=>$("GET",`/productos/${t}`),partners:(t=1)=>$("GET",`/partners?pagina=${t}`),partner:t=>$("GET",`/partners/${t}`),clientes:(t=1)=>$("GET",`/clientes?pagina=${t}`),proveedores:(t=1)=>$("GET",`/proveedores?pagina=${t}`),stock:(t=1)=>$("GET",`/stock?pagina=${t}`),stockKpis:()=>$("GET","/stock/kpis"),stockBajo:()=>$("GET","/stock/bajo"),stockProducto:t=>$("GET",`/stock/producto/${t}`),cfdiTimbrados:(t=1)=>$("GET",`/cfdi/timbrados?pagina=${t}`),cfdiTimbrado:t=>$("GET",`/cfdi/timbrados/${t}`),cfdiKpis:()=>$("GET","/cfdi/kpis"),timbrar:t=>$("POST","/cfdi/timbrar",t),cancelarCfdi:t=>$("POST","/cfdi/cancelar",t),nomina:(t=1)=>$("GET",`/nomina?pagina=${t}`),nominaKpis:()=>$("GET","/nomina/kpis"),compras:(t=1)=>$("GET",`/compras?pagina=${t}`),comprasKpis:()=>$("GET","/compras/kpis"),searchSync:()=>$("POST","/search/sync",{}),searchStatus:()=>$("GET","/search/status"),health:()=>$("GET","/health")};function Dt(){const t=document.getElementById("__shell");t&&t.remove(),document.getElementById("app").innerHTML=`
  <div class="login-bg">
  ...
  </div>`;const e=document.getElementById("lbtn"),r=document.getElementById("lu"),a=document.getElementById("lp"),i=document.getElementById("lerr");async function c(){if(e.disabled)return;const s=r.value.trim(),b=a.value;if(!s||!b){i.textContent="Ingresa usuario y contraseña",i.classList.add("show");return}e.disabled=!0,e.textContent="Verificando...",i.classList.remove("show");try{const o=await h.login(s,b),d=(o==null?void 0:o.data)||o,y=(d==null?void 0:d.access_token)||(d==null?void 0:d.token);if(y){tt.setSession(y,{nombre:d.email||s,email:d.email||s,user_id:d.user_id,company_id:d.company_id}),document.getElementById("app").innerHTML="",et("dashboard");return}i.textContent="Error inesperado del servidor. Intenta de nuevo.",i.classList.add("show")}catch(o){i.textContent=(o==null?void 0:o.status)===401?"Credenciales incorrectas. Verifica tu usuario y contraseña.":`Error de conexión: ${(o==null?void 0:o.message)||"No se pudo contactar el servidor"}`,i.classList.add("show")}e.disabled=!1,e.textContent="Acceder al sistema"}e.addEventListener("click",c),a.addEventListener("keydown",s=>s.key==="Enter"&&c()),r.addEventListener("keydown",s=>s.key==="Enter"&&a.focus()),setTimeout(()=>r.focus(),100)}function G(t,e=0){return t==null||t===""?"—":Number(t).toLocaleString("es-MX",{minimumFractionDigits:e,maximumFractionDigits:e})}function E(t){return t==null?"—":(t=parseFloat(t)||0,Math.abs(t)>=1e6?`$${(t/1e6).toFixed(2)}M`:Math.abs(t)>=1e3?`$${(t/1e3).toFixed(1)}k`:`$${G(t,2)}`)}function it(t){return t==null?"—":Number(t).toLocaleString("es-MX")}function F(t){return t?new Date(t).toLocaleDateString("es-MX",{day:"2-digit",month:"short",year:"numeric"}):"—"}function I(t,e="",r="info"){const a={success:"✅",error:"❌",info:"ℹ️",warning:"⚠️"};let i=document.getElementById("__toasts");i||(i=document.createElement("div"),i.id="__toasts",i.className="toast-container",document.body.appendChild(i));const c=document.createElement("div");c.className=`toast ${r}`,c.innerHTML=`
    <span class="toast-icon">${a[r]||"ℹ️"}</span>
    <div><div class="toast-title">${t}</div>${e?`<div class="toast-msg">${e}</div>`:""}</div>`,i.appendChild(c),requestAnimationFrame(()=>c.classList.add("show")),setTimeout(()=>{c.classList.remove("show"),setTimeout(()=>c.remove(),400)},3800)}function ut(t,e,r=900,a="",i=""){if(!t)return;const c=performance.now(),s=String(e).includes(".");function b(o){const d=Math.min((o-c)/r,1),y=1-Math.pow(1-d,3),x=e*y;t.textContent=a+(s?x.toLocaleString("es-MX",{minimumFractionDigits:2,maximumFractionDigits:2}):Math.round(x).toLocaleString("es-MX"))+i,d<1&&requestAnimationFrame(b)}requestAnimationFrame(b)}function Rt(t){if(!(t!=null&&t.length))return"";const e=Math.max(...t,1);return`<div class="sparkline">${t.map((r,a)=>`<div class="spark-bar${a===t.length-1?" active":""}" style="height:${Math.max(4,Math.round(r/e*100))}%"></div>`).join("")}</div>`}function zt(t=5,e=6){return`<tbody>${Array.from({length:e},()=>`<tr>${Array.from({length:t},()=>`<td><div class="skeleton" style="height:14px;width:${60+Math.random()*30}%"></div></td>`).join("")}</tr>`).join("")}</tbody>`}function S(t=5,e=4){return`<table class="data-table"><thead><tr>${Array.from({length:e},()=>`<th><div class="skeleton" style="height:12px;width:${40+Math.random()*40}%"></div></th>`).join("")}</tr></thead>${zt(e,t)}</table>`}function At(t=5){return Array.from({length:t},()=>`
  ...
  </div>`).join("")}const Nt={sale:"emerald",done:"indigo",draft:"gray",sent:"sky",cancel:"red",posted:"emerald",in_payment:"violet",paid:"emerald",partial:"amber"};function U(t,e){return`<span class="badge badge-${Nt[t]||"gray"} badge-dot">${e}</span>`}function N(t,e,r){return window.__pagNav=r,`
  <div class="data-table-footer">
    ...
    <div class="pagination">
  ...
  </div>`}let O=null;function Ht(t,e,r={}){let a=document.getElementById("__modal-overlay");a||(a=document.createElement("div"),a.id="__modal-overlay",a.innerHTML=`
      ...
      </div>`,document.body.appendChild(a),a.addEventListener("click",i=>{i.target===a&&window.__closeModal()})),document.getElementById("__modal-title").textContent=t,document.getElementById("__modal-body").innerHTML=e,a.classList.add("open"),document.body.style.overflow="hidden",O&&document.removeEventListener("keydown",O),O=i=>{i.key==="Escape"&&window.__closeModal()},document.addEventListener("keydown",O),r.onMounted&&setTimeout(r.onMounted,10)}function Vt(){const t=document.getElementById("__modal-overlay");t&&t.classList.remove("open"),document.body.style.overflow="",O&&(document.removeEventListener("keydown",O),O=null)}window.__closeModal=Vt;async function H(t,e,r){Ht(t,`
    <div style="display:flex;flex-direction:column;gap:12px;padding:8px 0">
      ${[1,2,3,4,5].map(()=>'<div class="skeleton" style="height:52px;border-radius:10px"></div>').join("")}
    </div>`);try{const a=await e(),i=(a==null?void 0:a.data)??a;document.getElementById("__modal-body").innerHTML=r(i)}catch(a){document.getElementById("__modal-body").innerHTML=`<p style="color:var(--red);padding:24px">Error: ${a.message}</p>`}}function p(t,e,r={}){const a=e??"—",i=r.color?`color:${r.color}`:"";return`
  <div style="display:flex;justify-content:space-between;align-items:flex-start;
    padding:10px 0;border-bottom:1px solid var(--border)">
    <span style="font-size:12px;color:var(--text-400);font-weight:600;min-width:140px">${t}</span>
    <span style="font-size:13px;font-weight:500;text-align:right;${i}">${a}</span>
  ...
  <div style="margin-bottom:20px">
    ...
    ${e}
  </div>`}const vt=[{id:"dashboard",icon:"📊",label:"Dashboard",section:"Principal"},{id:"ventas",icon:"💰",label:"Ventas",section:"Principal"},{id:"facturas",icon:"🧾",label:"Facturación",section:"Principal"},{id:"productos",icon:"📦",label:"Productos",section:"Principal"},{id:"partners",icon:"👥",label:"Clientes",section:"Principal"},{id:"stock",icon:"🏭",label:"Inventario",section:"Principal"},{id:"cfdi",icon:"🔏",label:"CFDI 4.0",section:"Fiscal",badge:"NUEVO"},{id:"nomina",icon:"👔",label:"Nómina IMSS",section:"Fiscal"},{id:"compras",icon:"🛒",label:"Compras",section:"Operaciones"},{id:"search",icon:"🔍",label:"NexusSearch",section:"Sistema"},{id:"reportes",icon:"📈",label:"Reportes",section:"Sistema"}];function M(){if(document.getElementById("__shell"))return;const t=tt.getUser(),e=(t.nombre||t.name||"AD").substring(0,2).toUpperCase(),r=[...new Set(vt.map(a=>a.section))];document.getElementById("app").innerHTML=`
  ...
  </div>`,window._go=a=>{et(a)},window._logout=()=>{tt.clear();const a=document.getElementById("__shell");a&&a.remove(),et("login"),I("Sesión cerrada","Hasta pronto","info")},window.addEventListener("hashchange",bt),bt()}function L(t){const e=document.getElementById("__page");e&&(e.innerHTML=t,e.scrollTop=0)}function B(t){const e=document.getElementById("__breadcrumb");e&&(e.innerHTML=t.map((r,a)=>`
    <span class="breadcrumb-item"${a<t.length-1&&r.href?` onclick="window._go('${r.href}')"`:""}>
      ${r.label}
      ${a<t.length-1?'<span class="breadcrumb-sep">/</span>':""}
    </span>`).join(""))}function bt(){const t=window.location.hash.replace("#","")||"dashboard";document.querySelectorAll(".nav-link").forEach(e=>{e.classList.toggle("active",e.id===`nl-${t}`)})}const Gt={sale:"indigo",done:"emerald",draft:"gray",cancel:"red",sent:"sky",posted:"emerald"},Ot={sale:"Confirmada",done:"Entregada",draft:"Borrador",cancel:"Cancelada",sent:"Enviada"};function K(t,e=10){return Array.from({length:e},()=>Math.max(5,Math.round(t*(.6+Math.random()*.8))))}async function wt(){var t,e,r,a,i,c,s,b,o;M(),B([{label:"Dashboard"}]),L(`
  ...
  <!-- KPI Cards skeleton -->
  <div class="kpi-grid anim-2" id="kpi-grid">${At(5)}</div>
  ...
  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px" class="anim-4">
    ...
    <div class="data-card" style="padding:20px">
      ...
      <div id="resumen-fiscal">${S(4,2)}</div>
    </div>
    ...
    <div class="data-card" style="padding:20px">
      ...
      <div id="system-status">${S(4,2)}</div>
    </div>
  </div>`);try{const[d,y,x]=await Promise.allSettled([h.dashboard(),h.ventas(1),h.stockBajo()]),l=d.status==="fulfilled"?(t=d.value)==null?void 0:t.data:null,n=[{key:"ventas_mes",label:"Ventas del Mes",tipo:"mxn",icon:"💰",color:"indigo",valor:parseFloat(((e=l==null?void 0:l.ventas)==null?void 0:e.importe_mes)||0),trend:null,spark:K(100)},{key:"facturas",label:"Facturas Emitidas",tipo:"num",icon:"🧾",color:"emerald",valor:parseInt(((r=l==null?void 0:l.facturacion)==null?void 0:r.total_facturas)||0),trend:null,spark:K(50)},{key:"cobrar",label:"Por Cobrar",tipo:"mxn",icon:"📋",color:"amber",valor:parseFloat(((a=l==null?void 0:l.facturacion)==null?void 0:a.por_cobrar)||0),trend:null,spark:K(80)},{key:"stock_total",label:"Productos en Stock",tipo:"num",icon:"📦",color:"sky",valor:parseInt(((i=l==null?void 0:l.inventario)==null?void 0:i.total_productos_con_stock)||0),trend:null,spark:K(80)},{key:"stock_bajo",label:"Alertas Stock Bajo",tipo:"num",icon:"⚠️",color:"rose",valor:parseInt(((c=l==null?void 0:l.inventario)==null?void 0:c.alertas_stock_bajo)||0),trend:null,spark:K(20)}],u=document.getElementById("kpi-grid");u&&(u.innerHTML=n.map(f=>`
      <div class="kpi-card kpi-${f.color}">
        ...
        ${Rt(f.spark)}
      </div>`).join(""),n.forEach(f=>{const _=document.getElementById("kv-"+f.key);_&&(f.tipo==="mxn"?ut(_,f.valor,1100,"$"):ut(_,f.valor,1100))}));const m=document.getElementById("tabla-ventas");if(m){const f=y.status==="fulfilled"?(((s=y.value)==null?void 0:s.data)||[]).slice(0,6):[];f.length===0?m.innerHTML='<p style="text-align:center;color:var(--text-400);padding:24px">Sin ventas registradas</p>':m.innerHTML=`
        <table class="data-table">
          <thead><tr>
            <th>Folio</th><th>Cliente</th><th>Fecha</th><th>Total</th><th>Estado</th>
          </tr></thead>
          <tbody>
            ${f.map(_=>{const k=_.state||"draft",R=Ot[k]||k,P=Gt[k]||"gray",V=_.date_order?new Date(_.date_order).toLocaleDateString("es-MX",{day:"2-digit",month:"short"}):"—";return`
              <tr>
                <td class="td-mono">${_.name||_.id}</td>
                <td class="td-primary">${_.partner_name||_.partner_id||"—"}</td>
                <td>${V}</td>
                <td class="td-amount">${E(parseFloat(_.amount_total||0))}</td>
        ...
        <table class="data-table">
          ...
          <tbody>
            ${f.map(_=>{const k=parseFloat(_.cantidad_disponible||0),R=k<=0?"red":k<5?"amber":"sky";return`
              <tr>
      ...
      </div>`).join("")}const w=document.getElementById("system-status");if(w){let f=!1;try{await h.health(),f=!0}catch{}w.innerHTML=[{label:"API Backend",val:f?"✅ En línea":"❌ Offline",color:f?"emerald":"red"},{label:"Base de datos",val:l?"✅ Operativa":"⚠️ Sin datos",color:l?"emerald":"amber"},{label:"Versión ERP",val:"v2.0.0",color:"indigo"},{label:"Uptime",val:"99.98%",color:"emerald"}].map(_=>`
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
        <span style="font-size:12.5px;color:var(--text-500)">${_.label}</span>
        <span class="badge badge-${_.color}">${_.val}</span>
      </div>`).join("")}}catch(d){console.error("Dashboard load error:",d),I("Error al cargar","No se pudo conectar con el servidor","error")}(o=document.getElementById("btn-refresh"))==null||o.addEventListener("click",()=>wt())}const gt={sale:{lbl:"Confirmada",color:"indigo"},done:{lbl:"Entregada",color:"emerald"},draft:{lbl:"Borrador",color:"gray"},cancel:{lbl:"Cancelada",color:"red"},sent:{lbl:"Enviada",color:"sky"}};let X=1,ft=0;async function qt(){M(),B([{label:"Dashboard",href:"dashboard"},{label:"Ventas"}]),X=1,await $t()}async function $t(){var t,e,r,a;L(`
  <div class="page-header anim-1">
    <div>
  ...
  <div class="data-card anim-3">
    ...
    <div id="ventas-tabla">${S(8,6)}</div>
  </div>`);try{const[i,c]=await Promise.allSettled([h.ventaKpis(),h.ventas(X)]),s=i.status==="fulfilled"?(t=i.value)==null?void 0:t.data:null,b=document.getElementById("kpi-row");b&&s&&(b.innerHTML=[{label:"Total Órdenes",val:s.ordenes_confirmadas??s.total_ordenes??0,tipo:"num",color:"indigo"},{label:"Facturado Total",val:s.total_facturado??0,tipo:"mxn",color:"emerald"},{label:"Ticket Promedio",val:s.ticket_promedio??0,tipo:"mxn",color:"violet"},{label:"Este Mes",val:s.ordenes_este_mes??0,tipo:"num",color:"amber"}].map(l=>`
      <div class="data-card" style="padding:16px">
      ...
      </div>`).join(""));const o=c.status==="fulfilled"?((e=c.value)==null?void 0:e.data)||[]:[];ft=((r=c.value)==null?void 0:r.total)??o.length;const d=o.length>=20,y=document.getElementById("ventas-sub");y&&(y.textContent=`${ft} registros · Página ${X}`);const x=document.getElementById("ventas-tabla");x&&(o.length===0?x.innerHTML='<p style="text-align:center;padding:32px;color:var(--text-400)">Sin ventas en esta página</p>':x.innerHTML=`
        <table class="data-table">
          <thead><tr>
            <th>Folio</th>
            <th>Cliente</th>
          ...
          <tbody>
            ${o.map(l=>{const n=gt[l.state]||{lbl:l.state||"—",color:"gray"},u=l.date_order?F(l.date_order):"—",m=l.invoice_status==="invoiced"?"Facturada":l.invoice_status==="to invoice"?"Por facturar":"—";return`
        ...
        ${N(X,d,l=>{X=l,$t()})}`),(a=document.getElementById("buscar-venta"))==null||a.addEventListener("input",l=>{const n=l.target.value.toLowerCase();document.querySelectorAll("#ventas-tabla tbody tr").forEach(u=>{u.style.display=u.textContent.toLowerCase().includes(n)?"":"none"})}),window._verVenta=l=>{H("Detalle de Orden de Venta",()=>h.get(`/ventas/${l}`),n=>{const u=gt[n.state]||{lbl:n.state,color:"gray"};return`
          ...
          </div>`})},window._nuevaVenta=()=>{I("Próximamente","Formulario de nueva venta en desarrollo","info")}}catch(i){console.error(i),I("Error al cargar ventas",i.message,"error");const c=document.getElementById("ventas-tabla");c&&(c.innerHTML=`<p style="text-align:center;padding:32px;color:var(--red)">Error de conexión: ${i.message}</p>`)}}const yt={posted:{lbl:"Publicada",color:"emerald"},draft:{lbl:"Borrador",color:"gray"},in_payment:{lbl:"En cobro",color:"violet"},paid:{lbl:"Pagada",color:"sky"},cancel:{lbl:"Cancelada",color:"red"}};let J=1;async function Ut(){M(),B([{label:"Dashboard",href:"dashboard"},{label:"Facturación"}]),J=1,await _t()}async function _t(){var t,e,r,a,i;L(`
  <div class="page-header anim-1">
    <div>
  ...
  </div>`);try{const[c,s,b]=await Promise.allSettled([h.factKpis(),h.facturas(J),h.porCobrar()]),o=c.status==="fulfilled"?(t=c.value)==null?void 0:t.data:null,d=document.getElementById("kpi-row");d&&(d.innerHTML=[{label:"Total Facturas",val:(o==null?void 0:o.total_facturas)||0,tipo:"num",color:"indigo",icon:"🧾"},{label:"Monto Facturado",val:(o==null?void 0:o.monto_total)||0,tipo:"mxn",color:"emerald",icon:"💰"},{label:"Por Cobrar",val:(o==null?void 0:o.por_cobrar)||0,tipo:"mxn",color:"amber",icon:"📋"},{label:"Facturas Vencidas",val:(o==null?void 0:o.facturas_vencidas)||0,tipo:"num",color:"red",icon:"⚠️"}].map(g=>`
      <div class="data-card" style="padding:16px">
        <div style="font-size:11px;color:var(--text-400);font-weight:600;margin-bottom:4px">${g.icon} ${g.label}</div>
        <div style="font-size:22px;font-weight:800;color:var(--text-900)">
          ${g.tipo==="mxn"?E(parseFloat(g.val)):it(parseInt(g.val))}
        </div>
      </div>`).join(""));const y=s.status==="fulfilled"?((e=s.value)==null?void 0:e.data)||[]:[],x=y.length>=20,l=document.getElementById("fact-sub");l&&(l.textContent=`${y.length} registros · Página ${J}`);const n=document.getElementById("fact-tabla");n&&(y.length===0?n.innerHTML='<p style="text-align:center;padding:32px;color:var(--text-400)">Sin facturas registradas</p>':n.innerHTML=`
        <table class="data-table">
          <thead><tr>
          ...
          <tbody>
            ${y.map(g=>{const v=yt[g.state]||{lbl:g.state||"—",color:"gray"},w=g.invoice_date||g.date?F(g.invoice_date||g.date):"—",f=g.partner_name&&isNaN(g.partner_name)?g.partner_name:g.customer_name||`Cliente #${g.partner_id}`;return`
        ...
        ${N(J,x,g=>{J=g,_t()})}`);const u=b.status==="fulfilled"?((r=b.value)==null?void 0:r.data)||[]:[],m=document.getElementById("por-cobrar-lista");m&&(u.length===0?m.innerHTML='<p style="color:var(--emerald);font-size:13px;text-align:center;padding:20px">✅ Sin saldo pendiente</p>':m.innerHTML=u.slice(0,8).map(g=>{const v=g.invoice_date_due&&new Date(g.invoice_date_due)<new Date;return`
          ...
          </div>`}).join("")),(a=document.getElementById("buscar-fact"))==null||a.addEventListener("input",g=>{const v=g.target.value.toLowerCase();document.querySelectorAll("#fact-tabla tbody tr").forEach(w=>{w.style.display=w.textContent.toLowerCase().includes(v)?"":"none"})}),(i=document.getElementById("filtro-estado"))==null||i.addEventListener("change",g=>{const v=g.target.value;document.querySelectorAll("#fact-tabla tbody tr").forEach(w=>{w.style.display=!v||w.dataset.estado===v?"":"none"})}),window._verFactura=g=>{H("Detalle de Factura",()=>h.factura(g),v=>{const w=yt[v.state]||{lbl:v.state,color:"gray"},f=v.partner_name&&isNaN(v.partner_name)?v.partner_name:`Cliente #${v.partner_id}`;return`
          ...
          </div>`})}}catch(c){console.error(c),I("Error al cargar facturas",c.message,"error")}}let q=1,z="";async function Kt(){M(),B([{label:"Dashboard",href:"dashboard"},{label:"Productos"}]),q=1,z="",await rt()}async function rt(){var t,e;L(`
  <div class="page-header anim-1">
    <div>
  ...
  <div class="data-card anim-2">
    ...
    <div id="prod-tabla">${S(10,6)}</div>
  </div>`);try{const r=await h.productos(q,z),a=(r==null?void 0:r.data)||[],i=a.length>=20,c=document.getElementById("prod-sub");c&&(c.textContent=`${a.length} productos${z?` para "${z}"`:""} · Página ${q}`);const s=document.getElementById("prod-tabla");s&&(a.length===0?s.innerHTML=`<p style="text-align:center;padding:40px;color:var(--text-400)">
        ...
        <table class="data-table">
          ...
          <tbody>
            ${a.map(o=>{const d=o.name&&typeof o.name=="object"?o.name.es_MX||o.name.en_US||Object.values(o.name)[0]||`Producto #${o.id}`:o.name||o.nombre||`Producto #${o.id}`,y=o.type_||o.type||"",x=y==="consu"?"Consumible":y==="service"?"Servicio":y==="product"?"Almacenable":"Consumible",l=y==="service"?"violet":y==="consu"?"sky":"indigo",n=E(parseFloat(o.list_price||o.precio||0)),u=o.active!==!1,m=o.categ_name||o.categoria||"",g=m==="Goods"?"Mercancía":m==="Services"?"Servicios":m||"—";return`
              <tr data-tipo="${y}" data-id="${o.id}" style="cursor:pointer" onclick="window._verProducto(${o.id})" title="Ver detalle">
                <td class="td-mono">${o.default_code||"—"}</td>
        ...
        ${N(q,i,o=>{q=o,rt()})}`);let b;(t=document.getElementById("buscar-prod"))==null||t.addEventListener("input",o=>{clearTimeout(b),b=setTimeout(()=>{z=o.target.value.trim(),q=1,rt()},400)}),(e=document.getElementById("filtro-tipo"))==null||e.addEventListener("change",o=>{const d=o.target.value;document.querySelectorAll("#prod-tabla tbody tr").forEach(y=>{y.style.display=!d||y.dataset.tipo===d?"":"none"})}),window._verProducto=o=>{const d=a.find(m=>m.id===o);if(!d)return;const y=d.name&&typeof d.name=="object"?d.name.es_MX||d.name.en_US||"":d.name||"",x=d.type_||d.type||"",l=x==="consu"?"Consumible":x==="service"?"Servicio":"Almacenable",n=d.categ_name||"",u=n==="Goods"?"Mercancía":n==="Services"?"Servicios":n||"—";H("Detalle de Producto",async()=>d,()=>`
        ...
        </div>`)}}catch(r){console.error(r),I("Error al cargar productos",r.message,"error")}}let A=1,D="";async function Xt(){M(),B([{label:"Dashboard",href:"dashboard"},{label:"Clientes / Proveedores"}]),A=1,D="",await lt()}async function lt(){var t,e,r;L(`
  <div class="page-header anim-1">
    <div>
  ...
  <div class="data-card anim-3">
    ...
    <div id="part-tabla">${S(10,5)}</div>
  </div>`),window._filterPart=a=>{D=a,A=1,lt()};try{let a;D==="clientes"?a=h.clientes(A):D==="proveedores"?a=h.proveedores(A):a=h.partners(A);const[i,c]=await Promise.allSettled([a,h.partners(1)]),s=i.status==="fulfilled"?((t=i.value)==null?void 0:t.data)||[]:[],b=c.status==="fulfilled"?((e=c.value)==null?void 0:e.data)||[]:s,o=s.length>=20,d=document.getElementById("stats-row");if(d){const l=b.filter(u=>(u.customer_rank||0)>0).length,n=b.filter(u=>(u.supplier_rank||0)>0).length;d.innerHTML=[{label:"Total Contactos",val:b.length,color:"indigo",icon:"📋"},{label:"Clientes",val:l,color:"emerald",icon:"👥"},{label:"Proveedores",val:n,color:"violet",icon:"🏭"}].map(u=>`
      <div class="data-card" style="padding:16px">
        <div style="font-size:11px;color:var(--text-400);font-weight:600;margin-bottom:4px">${u.icon} ${u.label}</div>
        <div style="font-size:26px;font-weight:800;color:var(--text-900)">${it(u.val)}</div>
      </div>`).join("")}const y=document.getElementById("part-sub");y&&(y.textContent=`${s.length} contactos · Página ${A}`);const x=document.getElementById("part-tabla");x&&(s.length===0?x.innerHTML='<p style="text-align:center;padding:32px;color:var(--text-400)">Sin contactos registrados</p>':x.innerHTML=`
        <table class="data-table">
          <thead><tr>
            <th>Nombre</th><th>Tipo</th><th>Email</th><th>Teléfono</th><th>Tags</th>
          </tr></thead>
          <tbody>
            ${s.map(l=>{const n=(l.customer_rank||0)>0,u=(l.supplier_rank||0)>0,m=l.is_company;return`
        ...
        ${N(A,o,l=>{A=l,lt()})}`),(r=document.getElementById("buscar-part"))==null||r.addEventListener("input",l=>{const n=l.target.value.toLowerCase();document.querySelectorAll("#part-tabla tbody tr").forEach(u=>{u.style.display=u.textContent.toLowerCase().includes(n)?"":"none"})}),window._verPartner=l=>{H("Detalle de Contacto",()=>h.partner(l),n=>{const u=(n.customer_rank||0)>0,m=(n.supplier_rank||0)>0;return`
          ...
          </div>`})}}catch(a){console.error(a),I("Error al cargar contactos",a.message,"error")}}const Jt=["deposit","down payment","downpayment","pago inicial"];let Y=1;async function Yt(){M(),B([{label:"Dashboard",href:"dashboard"},{label:"Inventario"}]),Y=1,await Et()}async function Et(){var t,e,r,a,i;L(`
  <div class="page-header anim-1">
    <div>
  ...
  <div style="display:grid;grid-template-columns:1fr 340px;gap:16px" class="anim-3">
    ...
    <div class="data-card" style="padding:16px">
  ...
  </div>`);try{const[c,s,b]=await Promise.allSettled([h.stockKpis(),h.stock(Y),h.stockBajo()]),o=c.status==="fulfilled"?(t=c.value)==null?void 0:t.data:null,d=document.getElementById("kpi-row");d&&o&&(d.innerHTML=[{label:"Con stock",val:o.total_productos_con_stock||0,tipo:"num",color:"emerald",icon:"✅"},{label:"Sin stock",val:o.total_sin_stock||0,tipo:"num",color:"red",icon:"❌"},{label:"Valor Inventario",val:o.valor_inventario||0,tipo:"mxn",color:"indigo",icon:"💰"},{label:"Alertas Bajo",val:o.alertas_stock_bajo||0,tipo:"num",color:"amber",icon:"⚠️"}].map(v=>`
      <div class="data-card" style="padding:16px">
      ...
      </div>`).join(""));const y=s.status==="fulfilled"?((e=s.value)==null?void 0:e.data)||[]:[],x=y.filter(v=>{const w=(v.product_name||"").toLowerCase();return!Jt.some(f=>w.includes(f))}),l=y.length>=20,n=document.getElementById("stock-sub");n&&(n.textContent=`${x.length} productos · Página ${Y}`);const u=document.getElementById("stock-tabla");u&&(x.length===0?u.innerHTML='<p style="text-align:center;padding:32px;color:var(--text-400)">Sin datos de stock</p>':u.innerHTML=`
        <table class="data-table">
          <thead><tr>
            <th>Producto</th>
            <th>Disponible</th>
          ...
          <tbody>
            ${x.map(v=>{const w=parseFloat(v.cantidad_disponible||0),f=parseFloat(v.cantidad_reservada||0),_=w<=0?"red":w<10?"amber":"emerald",k=w<=0?"❌ Sin stock":w<10?"⚠️ Stock bajo":"✅ Normal";return`
              <tr data-alerta="${w<10?"bajo":"ok"}" style="cursor:pointer" onclick="window._verStock(${v.product_id})" title="Ver detalle">
                ...
                <td><span class="badge badge-${_}">${G(w,0)}</span></td>
                <td style="color:var(--text-400)">${G(f,0)}</td>
                <td class="td-mono" style="font-size:11px">${v.ubicacion||"—"}</td>
                <td><span class="badge badge-${_}">${k}</span></td>
              </tr>`}).join("")}
          </tbody>
        </table>
        ${N(Y,l,v=>{Y=v,Et()})}`);const m=b.status==="fulfilled"?((r=b.value)==null?void 0:r.data)||[]:[],g=document.getElementById("stock-bajo-lista");g&&(m.length===0?g.innerHTML='<p style="color:var(--emerald);font-size:13px;text-align:center;padding:16px">✅ Todo en niveles normales</p>':g.innerHTML=m.map(v=>{const w=parseFloat(v.cantidad_disponible||0),f=w<=0?"red":"amber";return`
          <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid var(--border)">
            <div>
          ...
          </div>`}).join("")),(a=document.getElementById("buscar-stock"))==null||a.addEventListener("input",v=>{const w=v.target.value.toLowerCase();document.querySelectorAll("#stock-tabla tbody tr").forEach(f=>{f.style.display=f.textContent.toLowerCase().includes(w)?"":"none"})}),(i=document.getElementById("filtro-stock"))==null||i.addEventListener("change",v=>{const w=v.target.value;document.querySelectorAll("#stock-tabla tbody tr").forEach(f=>{if(w==="todos"){f.style.display="";return}const _=f.dataset.alerta;f.style.display=_===w?"":"none"})}),window._verStock=v=>{H("Detalle de Stock",()=>h.stockProducto(v),w=>{const f=Array.isArray(w)?w:[w],_=f[0]||{},k=parseFloat(_.cantidad_disponible||0),R=parseFloat(_.cantidad_reservada||0),P=k<=0?"var(--red)":k<10?"var(--warning)":"var(--success)";return`
          ${C("Producto",[p("Nombre",_.product_name||`#${v}`),p("Cantidad disponible",`<strong style="color:${P}">${G(k,2)}</strong>`),p("Cantidad reservada",G(R,2)),p("Cantidad neta",G(k-R,2))].join(""))}
          ${f.length>1?C("Por ubicación",f.map(V=>p(V.ubicacion||"Sin ubicación",G(parseFloat(V.cantidad_disponible||0),2))).join("")):C("Ubicación",[p("Almacén",_.ubicacion||"Sin ubicación")].join(""))}
          <div style="display:flex;gap:10px;margin-top:16px">
            <button class="btn btn-secondary btn-sm" onclick="window.__closeModal()">Cerrar</button>
            <button class="btn btn-primary btn-sm" onclick="alert('Ajuste de inventario — próximamente')">📋 Ajustar</button>
          </div>`})}}catch(c){console.error(c),I("Error al cargar inventario",c.message,"error")}}let W=1,at="historial";async function Wt(){M(),B([{label:"Dashboard",href:"dashboard"},{label:"CFDI 4.0"}]),W=1,await kt()}async function kt(){L(`
  <div class="page-header anim-1">
    <div>
  ...
  <div class="data-card anim-3" id="cfdi-content">
    <div id="cfdi-body">${S(6,6)}</div>
      ...
      </div>`).join("")),at==="historial"?await dt():Qt()}catch(t){console.error(t),I("Error CFDI",t.message,"error")}}async function dt(){const t=document.getElementById("cfdi-body");t&&(t.innerHTML=S(6,6));const e=await h.cfdiTimbrados(W).catch(()=>({data:[],total:0})),r=(e==null?void 0:e.data)||[],a=(e==null?void 0:e.total)??r.length,i=r.length>=20,c=document.getElementById("cfdi-sub");if(c&&(c.textContent=`${a} CFDIs timbrados · Página ${W}`),!!t){if(r.length===0){t.innerHTML=`
  ...
  <table class="data-table">
    ...
    <tbody>
      ${r.map(s=>{const b=s.estado==="vigente"?"emerald":s.estado==="cancelado"?"red":"gray";return`
        <tr style="cursor:pointer" onclick="window._verCfdi('${s.uuid}')" title="Ver detalle">
          ...
          <td><span class="badge badge-${b}">${s.estado||"—"}</span></td>
          <td style="font-size:12px">${F(s.fecha_timbrado||s.created_at)}</td>
        </tr>`}).join("")}
    </tbody>
  </table>
  ${N(W,i,s=>{W=s,dt()})}`,window._verCfdi=s=>{H("Detalle CFDI",()=>h.cfdiTimbrado(s),b=>`
      ...
      </div>`)},window._cancelarCfdi=async s=>{if(confirm(`¿Cancelar el CFDI ${s.substring(0,18)}…?`))try{await h.cancelarCfdi({uuid:s,rfc_emisor:"",motivo:"02"}),I("CFDI cancelado",s,"success"),window.__closeModal(),dt()}catch(b){I("Error al cancelar",b.message,"error")}}}}function Qt(){var e;const t=document.getElementById("cfdi-body");t&&(t.innerHTML=`
  <div class="data-card-header">
  ...
  </div>`,(e=document.getElementById("f-subtotal"))==null||e.addEventListener("input",r=>{const a=parseFloat(r.target.value)||0,i=a*.16;document.getElementById("f-iva").value=i.toFixed(2),document.getElementById("f-total").value=(a+i).toFixed(2)}),window._timbrar=async()=>{var i,c,s,b,o,d,y,x,l,n,u,m,g,v;const r=document.getElementById("btn-timbrar");r.textContent="⏳ Timbrando…",r.disabled=!0;const a=document.getElementById("cfdi-resultado");try{const w=(i=document.getElementById("f-cer"))==null?void 0:i.files[0],f=(c=document.getElementById("f-key"))==null?void 0:c.files[0],_=ct=>new Promise((pt,Ft)=>{if(!ct){pt("");return}const nt=new FileReader;nt.onload=Mt=>pt(Mt.target.result.split(",")[1]||""),nt.onerror=Ft,nt.readAsDataURL(ct)}),[k,R]=await Promise.all([_(w),_(f)]),P=parseFloat((s=document.getElementById("f-subtotal"))==null?void 0:s.value)||0,V=P*.16,Tt={cfdi:{serie:((b=document.getElementById("f-serie"))==null?void 0:b.value)||"A",folio:((o=document.getElementById("f-folio"))==null?void 0:o.value)||"1",tipo_comprobante:((d=document.getElementById("f-tipo"))==null?void 0:d.value)||"I",emisor:{rfc:((y=document.getElementById("f-rfc-emisor"))==null?void 0:y.value)||"",nombre:((x=document.getElementById("f-nombre-emisor"))==null?void 0:x.value)||"",regimen_fiscal:((l=document.getElementById("f-regimen"))==null?void 0:l.value)||"601"},receptor:{rfc:((n=document.getElementById("f-rfc-receptor"))==null?void 0:n.value)||"",nombre:((u=document.getElementById("f-nombre-receptor"))==null?void 0:u.value)||"",uso_cfdi:((m=document.getElementById("f-uso"))==null?void 0:m.value)||"G03",domicilio_fiscal_receptor:"64000",regimen_fiscal_receptor:"601"},conceptos:[{clave_prod_serv:"84111506",descripcion:((g=document.getElementById("f-concepto"))==null?void 0:g.value)||"Servicio",cantidad:"1",unidad:"ACT",valor_unitario:P.toFixed(2),importe:P.toFixed(2),impuestos:{traslados:[{base:P.toFixed(2),impuesto:"002",tipo_factor:"Tasa",tasa:"0.160000",importe:V.toFixed(2)}]}}],subtotal:P.toFixed(2),total:(P+V).toFixed(2),moneda:"MXN",lugar_expedicion:"64000"},cert_b64:k,key_b64:R,password:((v=document.getElementById("f-pwd"))==null?void 0:v.value)||""},j=await h.timbrar(Tt);j!=null&&j.success?(a.innerHTML=`
  ...
  <div class="data-card anim-3">
    ...
    <div id="nom-tabla">${S(8,5)}</div>
      ...
      <div class="data-card" style="padding:16px">
      ...
      </div>`).join(""));const o=c.status==="fulfilled"?((e=c.value)==null?void 0:e.data)||[]:[],d=((r=c.value)==null?void 0:r.total)??o.length,y=o.length>=20,x=document.getElementById("nom-sub");x&&(x.textContent=`${d} empleados · Página ${Q}`);const l=document.getElementById("nom-tabla");l&&(o.length===0?l.innerHTML=`
        <div style="text-align:center;padding:60px 24px">
          <div style="font-size:48px;margin-bottom:16px">👔</div>
        ...
        <table class="data-table">
          ...
          <tbody>
            ${o.map(n=>{const u=n.active!==!1,m=(n.name||"?").split(" ").map(g=>g[0]).slice(0,2).join("");return`
        ...
        ${N(Q,y,n=>{Q=n,Ct()})}`),(a=document.getElementById("buscar-nom"))==null||a.addEventListener("input",n=>{const u=n.target.value.toLowerCase();document.querySelectorAll("#nom-tabla tbody tr").forEach(m=>{m.style.display=m.textContent.toLowerCase().includes(u)?"":"none"})}),window._verEmpleado=n=>{const u=o.find(m=>m.id===n);u&&H("Detalle de Empleado",async()=>u,m=>`
        ...
        </div>`)}}catch(i){console.error(i),I("Error al cargar nómina",i.message,"error")}}const ht={purchase:{lbl:"Confirmada",color:"indigo"},done:{lbl:"Recibida",color:"emerald"},draft:{lbl:"Borrador",color:"gray"},cancel:{lbl:"Cancelada",color:"red"},sent:{lbl:"Enviada",color:"sky"}};let Z=1;async function te(){M(),B([{label:"Dashboard",href:"dashboard"},{label:"Compras"}]),Z=1,await It()}async function It(){var t,e,r,a;L(`
  <div class="page-header anim-1">
    <div>
  ...
  <div class="data-card anim-3">
    ...
    <div id="comp-tabla">${S(8,5)}</div>
  </div>`);try{const[i,c]=await Promise.allSettled([h.comprasKpis(),h.compras(Z)]),s=i.status==="fulfilled"?(t=i.value)==null?void 0:t.data:null,b=document.getElementById("kpi-row");b&&(b.innerHTML=[{label:"Total OC",val:(s==null?void 0:s.total)??0,tipo:"num",color:"indigo",icon:"📋"},{label:"Confirmadas",val:(s==null?void 0:s.confirmadas)??0,tipo:"num",color:"emerald",icon:"✅"},{label:"Monto Total",val:(s==null?void 0:s.monto_total)??0,tipo:"mxn",color:"violet",icon:"💰"},{label:"Completadas",val:(s==null?void 0:s.completadas)??0,tipo:"num",color:"amber",icon:"📦"}].map(n=>`
      <div class="data-card" style="padding:16px">
        <div style="font-size:11px;color:var(--text-400);font-weight:600;margin-bottom:4px">${n.icon} ${n.label}</div>
        <div style="font-size:22px;font-weight:800;color:var(--text-900)">
          ${n.tipo==="mxn"?E(parseFloat(n.val)):Number(n.val).toLocaleString("es-MX")}
        </div>
      </div>`).join(""));const o=c.status==="fulfilled"?((e=c.value)==null?void 0:e.data)||[]:[],d=((r=c.value)==null?void 0:r.total)??o.length,y=o.length>=20,x=document.getElementById("comp-sub");x&&(x.textContent=`${d} órdenes · Página ${Z}`);const l=document.getElementById("comp-tabla");l&&(o.length===0?l.innerHTML='<div style="text-align:center;padding:60px;color:var(--text-400)">Sin órdenes de compra registradas</div>':l.innerHTML=`
        <table class="data-table">
          <thead><tr>
          ...
          <tbody>
            ${o.map(n=>{const u=ht[n.state]||{lbl:n.state||"—",color:"gray"};return`
        ...
        ${N(Z,y,n=>{Z=n,It()})}`),(a=document.getElementById("buscar-comp"))==null||a.addEventListener("input",n=>{const u=n.target.value.toLowerCase();document.querySelectorAll("#comp-tabla tbody tr").forEach(m=>{m.style.display=m.textContent.toLowerCase().includes(u)?"":"none"})}),window._verCompra=n=>{const u=o.find(m=>m.id===n);u&&H("Detalle Orden de Compra",async()=>u,m=>{const g=ht[m.state]||{lbl:m.state,color:"gray"};return`
          ...
          </div>`})}}catch(i){console.error(i),I("Error al cargar compras",i.message,"error")}}let st=null;async function ee(){M(),B([{label:"Dashboard",href:"dashboard"},{label:"NexusSearch"}]),await ae()}async function ae(){var e,r;L(`
  <div class="page-header anim-1">
    <div>
  ...
  <!-- Estado del índice -->
  <div id="index-status" class="anim-4" style="margin-top:16px"></div>`),(e=document.getElementById("search-query"))==null||e.addEventListener("keydown",a=>{a.key==="Enter"&&window._buscar()});let t;(r=document.getElementById("search-query"))==null||r.addEventListener("input",a=>{clearTimeout(t),!(a.target.value.length<2)&&(t=setTimeout(()=>window._buscar(),500))}),await xt(),window._buscar=oe,window._checkStatus=xt,window._syncSearch=ie}async function oe(){var r,a;const t=(a=(r=document.getElementById("search-query"))==null?void 0:r.value)==null?void 0:a.trim();if(!t||t.length<2)return;const e=document.getElementById("search-results");e&&(e.innerHTML=`
    <div class="data-card" style="padding:20px;text-align:center;color:var(--text-400)">
      <div class="spinner" style="margin:0 auto 8px"></div>
      <div>Buscando "${t}"…</div>
    </div>`);try{const[i,c,s]=await Promise.allSettled([h.ventas(1).then(o=>((o==null?void 0:o.data)||[]).filter(d=>(d.name||"").toLowerCase().includes(t.toLowerCase())||(d.partner_name||"").toLowerCase().includes(t.toLowerCase())).map(d=>({tipo:"Venta",icon:"💰",titulo:d.name,sub:d.partner_name,meta:`$${d.amount_total}`,href:"ventas"}))),h.productos(1,t).then(o=>((o==null?void 0:o.data)||[]).map(d=>{var y,x;return{tipo:"Producto",icon:"📦",titulo:typeof d.name=="object"?((y=d.name)==null?void 0:y.es_MX)||((x=d.name)==null?void 0:x.en_US)||"":d.name||"",sub:d.categ_name||"",meta:"",href:"productos"}})),h.partners(1).then(o=>((o==null?void 0:o.data)||[]).filter(d=>(d.name||"").toLowerCase().includes(t.toLowerCase())||(d.email||"").toLowerCase().includes(t.toLowerCase())).map(d=>({tipo:"Contacto",icon:"👥",titulo:d.name,sub:d.email||"",meta:"",href:"partners"})))]),b=[...i.status==="fulfilled"?i.value:[],...c.status==="fulfilled"?c.value:[],...s.status==="fulfilled"?s.value:[]];if(!e)return;if(b.length===0){e.innerHTML=`
    ...
    <div class="data-card">
      ...
      <div style="padding:0 4px">
        ...
        <div style="display:flex;align-items:center;gap:12px;padding:12px 8px;
          ...
          onclick="window._go('${o.href}')">
          <div style="width:36px;height:36px;border-radius:10px;background:var(--primary-light);
            display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">
            ${o.icon}
          </div>
          <div style="flex:1">
            <div style="font-weight:600;color:var(--text-800);font-size:13px">${o.titulo}</div>
            <div style="font-size:11px;color:var(--text-400)">${o.sub}</div>
          </div>
          <div style="display:flex;align-items:center;gap:8px">
            ${o.meta?`<span style="font-size:12px;font-weight:700;color:var(--text-700)">${o.meta}</span>`:""}
            <span class="badge badge-${o.tipo==="Venta"?"indigo":o.tipo==="Producto"?"emerald":"violet"}">${o.tipo}</span>
          </div>
        </div>`).join("")}
      </div>
    </div>`}catch(i){console.error(i),e&&(e.innerHTML=`<p style="color:var(--red);padding:20px">Error: ${i.message}</p>`)}}async function xt(){const t=document.getElementById("index-status");try{const e=await h.searchStatus().catch(()=>null);st=(e==null?void 0:e.data)||e,t&&st&&(t.innerHTML=`
      <div class="data-card" style="padding:16px">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px">
          ${Object.entries(st).map(([r,a])=>`
          <div style="padding:12px;background:var(--bg);border-radius:10px;border:1px solid var(--border)">
            <div style="font-size:10px;color:var(--text-400);font-weight:700;text-transform:uppercase;margin-bottom:4px">${r}</div>
            <div style="font-size:13px;font-weight:600;color:var(--text-800)">${JSON.stringify(a)}</div>
          </div>`).join("")}
        </div>
      </div>`)}catch{t&&(t.innerHTML="")}}async function ie(){const t=document.getElementById("btn-sync");t&&(t.textContent="⏳ Sincronizando…",t.disabled=!0);try{const e=await h.searchSync();I("Sincronización iniciada",(e==null?void 0:e.message)||"Los índices se están actualizando","success")}catch(e){I("Error de sincronización",e.message,"error")}finally{t&&(t.textContent="⚡ Sincronizar Índices",t.disabled=!1)}}async function ne(){M(),B([{label:"Dashboard",href:"dashboard"},{label:"Reportes"}]),await se()}async function se(){L(`
  <div class="page-header anim-1">
    <div>
  ...
  <div class="data-card anim-3">
    ...
    <div id="rep-contenido">
  ...
  </div>`),window._verReporte=t=>{I("Reporte seleccionado",`Generando reporte de ${t}…`,"info"),re(t)},window._exportReporte=()=>{I("Exportar","Función de exportación CSV/PDF — próximamente","info")},await St()}async function St(){var e,r,a,i;const t=document.getElementById("rep-fecha");t&&(t.textContent=new Date().toLocaleDateString("es-MX",{day:"2-digit",month:"long",year:"numeric"}));try{const[c,s,b,o]=await Promise.allSettled([h.ventaKpis(),h.factKpis(),h.stockKpis(),h.comprasKpis()]),d=((e=c.value)==null?void 0:e.data)||{},y=((r=s.value)==null?void 0:r.data)||{},x=((a=b.value)==null?void 0:a.data)||{},l=((i=o.value)==null?void 0:i.data)||{},n=document.getElementById("rep-kpis");n&&(n.innerHTML=`
      ...
      </div>`)}catch(c){console.error(c)}}async function re(t){const e=document.getElementById("rep-kpis"),r=document.querySelector(".data-card-title");if(r){const a={ventas:"💰 Reporte de Ventas",facturas:"🧾 Facturación",inventario:"🏭 Inventario",compras:"🛒 Compras",clientes:"👥 Clientes",nomina:"👔 Nómina"};r.textContent=a[t]||"Reporte"}e&&(e.innerHTML='<div class="skeleton" style="height:120px;border-radius:12px;grid-column:1/-1"></div>'),await St()}function le(t,e,r,a){M(),B([{label:"Dashboard",href:"dashboard"},{label:e}]),L(`
  <div class="page-header anim-1">
    <div>
  ...
  </div>`)}T("login",Dt);T("dashboard",wt);T("ventas",qt);T("facturas",Ut);T("productos",Kt);T("partners",Xt);T("stock",Yt);T("cfdi",Wt);T("nomina",Zt);T("compras",te);T("search",ee);T("reportes",ne);T("404",()=>le("404","Página no encontrada","La ruta solicitada no existe","🔍"));Lt();
